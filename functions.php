...
function longestWord($sorted){
	$longest = 0;
	for($i=0; $i<sizeOf($sorted); $i++){
		if(sizeOf($sorted[$i]) > $longest){
			$longest = sizeOf($sorted);
		}
	}
	return $longest;
}

function numberWords($sorted){
	$total = 0;
...
function averageLength($total, $sorted){
	return $total/sizeOf($sorted);
}

function removeCommon($sorted){
	$common = array("the", "a", "of", "to", "and", "or", "is", "in", "that", "as", "are", "I");
	foreach($sorted as $key){
		if(in_array($key, $common)){ unset($key, $sorted);} 
	}
	return $sorted;
}
...
<?php 
//printSecondGraph($sorted);
} 


// PRINTS GRAPH GIVEN SORTED ASSOCIATIVE ARRAY, HTML FILE MUST CONTAIN CANVAS! ADD CANVAS VARIABLE FOR MULTIPLE CANVI
function printSecondGraph($sorted){ 
        $sorted = removeCommon($sorted);
        ?>
...
function printTotals($sorted){ ?>
	<div class="graph">
  <div class="headerLabel">Highest Frequency Words In All Documents</div>
	<script>
...
function getAggregate(){
    ...
    try {
		...
		$array = array();
		foreach( $stmt as $row ) {
		        $a1 = unserialize($row['array']);
			foreach (array_keys($a1) as $key) {
    			    $array[$key] = @($a1[$key] + $array[$key]);
			}
		}	
